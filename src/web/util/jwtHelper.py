...
import datetime
from util.constant import private_key, public_key, jwt_algorithm


def generate_token(payload):
    return jwt.generate_jwt(payload, private_key, jwt_algorithm, datetime.timedelta(minutes=1))


def decode_token(token):
    return jwt.verify_jwt(token, public_key, [jwt_algorithm])
