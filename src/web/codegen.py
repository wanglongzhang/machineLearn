...
from util.constant import CODE_TEMPLATE_DIR, DAL_DIR, CONTROLLER_DIR
...
def auto_gen_dao_code(env, table_name_list):
    template = env.get_template('Dao.tmpl')
    for each in table_name_list:
        print(template.render(model_name=each))
        with open(os.path.join(DAL_DIR, "{}Dao.py".format(each)), "w") as fp:
            fp.write(template.render(model_name=each, lowercase_model_name=each.lower()))


def auto_gen_controller_code(env, table_name_list):
    template = env.get_template('Controller.tmpl')
    for each in table_name_list:
        print(template.render(model_name=each, lowercase_model_name=each.lower()))
        with open(os.path.join(CONTROLLER_DIR, "{}Controller.py".format(each)), "w") as fp:
            fp.write(template.render(model_name=each, lowercase_model_name=each.lower()))
...
if __name__ == "__main__":
    ...
    #auto_gen_dao_code(env, table_name_list)

    auto_gen_controller_code(env, table_name_list)